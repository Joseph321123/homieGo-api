...
-- Usuario anfitriona y propiedades de demostración
...
WHERE u.email = 'ana@homiego.demo'
  AND NOT EXISTS (
    SELECT 1 FROM propiedades WHERE titulo = 'Casa de playa con terraza'
  );
...
WHERE u.email = 'ana@homiego.demo'
  AND NOT EXISTS (
    SELECT 1 FROM propiedades WHERE titulo = 'Suite moderna en el centro'
  );
...
WHERE u.email = 'ana@homiego.demo'
  AND NOT EXISTS (
    SELECT 1 FROM propiedades WHERE titulo = 'Cabaña tranquila para descansar'
  );
...
WHERE p.titulo = 'Casa de playa con terraza'
  AND NOT EXISTS (
    SELECT 1 FROM fotos_propiedad f WHERE f.propiedad_id = p.id
  );
